...
const CellArea = (props: CellContext<BitacoraRecord, unknown>) => {
  ...
  return (
    <Badge
      ...
      className="w-full rounded-full border-none text-zinc shadow-xl bg-current/10 focus-visible:outline-none"
    >
      {value !== "N/A" ? <Building className="shrink-0" size={300} /> : <Ban />}
      <p className="max-w-30 truncate hover:whitespace-normal hover:text-clip text-center hover:max-w-full">{value ? value : "N/A"}</p>
    </Badge>
  )
}
